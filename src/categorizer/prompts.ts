...
const ACTIVITY_REPRODUCE_TEMPLATE = [
  ...
  "  once, then one `add_log` per site the code summary already named — the branch that returns",
  ...
  "  Placed a probe in the wrong place? `remove_log {logId}` takes that one back out — it is noise in",
  "  every later collect otherwise. This step comes BEFORE the run and cannot be done after it: a",
  "  probe has to be in the code that gets built, so adding one later costs you the whole build again.",
  "",
  ...
  "",
  "  `activity_trace_start`  Opens the session. Returns the traceId and the marker line for THIS",
  "                          project's language — a plain print/log statement starting with the",
  "                          session's own `TURING_TRACE_<suffix>`; no helper and no import to add.",
  "                          Once per pass.",
