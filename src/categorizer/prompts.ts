...
const ACTIVITY_REPRODUCE_TEMPLATE = [
  ...
  "  1. INSTRUMENT — and decide this BEFORE you launch, because a probe has to be compiled in:",
  "     is the defect VISIBLE (wrong text, wrong colour, broken layout, a crash) or is it a VALUE",
  "     THAT NEVER ARRIVES (a status that does not repaint, a list that does not refresh, a callback",
  "     that does not fire)? Visible ⇒ a capture is your evidence, skip to 2. Invisible ⇒ a",
  "     screenshot of a screen that looks normal proves NOTHING: `activity_trace_start` (once), then",
  "     `add_log` at the sites the code summary named — the branch that returns early, the notify",
  "     that is not called. Reproducing an invisible defect \"visually\" is not hard, it is impossible;",
  "     that is what probes are for, and after the build starts they cost you the whole build again.",
